...
public class Solution {
	public ListNode move(ListNode head, int step){
		for(int i=0;i<step&&head!=null;i++,head=head.next);
		return head;
	}
	...
	public ListNode sortList(ListNode head) {
        ...
        for(int i=1;i/2<=n;i*=2){
        	p=head; q=head; pre=head;
        	while(p!=null&&q!=null){
        		q=move(q,i);
        		pcount=0;qcount=0;
        		while((p!=null&& pcount<i)||(q!=null&&qcount<i)){
        			if(p==null||pcount>=i||(q!=null&& qcount<i && p.val>q.val)){
        				if(p==head&&pcount<i && q.val<head.val)
        					head=q;
        				if(pre!=p)
        					pre.next=q;
        				pre=q;
        			...
        			else if(q==null||qcount>=i|| p.val<=q.val){
        				if(pre!=p)
        					pre.next=p;
        				pre=p;
        				p=p.next;
